...
- client auth, onboarding, permissions, and active store state are restored from the canonical bootstrap payload at `GET /api/v1/me`.
...
4. Client bootstrap fetches `GET /api/v1/me` and hydrates the dashboard shell, onboarding state, permissions, and active store context from that payload.
...
- TanStack Query owns the canonical bootstrap query key `['bootstrap', 'me']`.
